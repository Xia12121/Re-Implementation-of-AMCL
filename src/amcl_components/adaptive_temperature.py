...
class AdaptiveTemperature(nn.Module):
    def __init__(self, input_dim, num_heads=3, temp_init=0.1, lower_limit=1e-5, upper_limit=2.0, beta=0.1):
        ...
        # 将 nn.ModuleList 更改为 nn.ParameterList
        # Learnable temperature for each head
        self.temperature_heads = nn.ParameterList([nn.Parameter(torch.Tensor([temp_init])) for _ in range(num_heads)])
    ...
    def forward(self, z_i_list, z_j_list):
        ...
        for head_idx in range(self.num_heads):
            ...
            temp = torch.sigmoid(dot_product.clamp(-10, 10)) * (self.upper_limit - self.lower_limit) + self.lower_limit
